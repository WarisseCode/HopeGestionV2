            ...
            <div className="space-y-6 h-full flex flex-col">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-[500px]">
                    <div className="lg:col-span-2 flex flex-col gap-4">
                        ...
                        <Card className="flex-1 flex flex-col p-4 bg-base-200 border border-base-300">
                             <label className="block text-sm font-bold text-base-content/80 mb-2">Contenu du document</label>
                             <textarea 
                                id="template-editor"
                                className="flex-1 w-full bg-base-100 border border-gray-300 rounded p-4 font-mono text-sm leading-relaxed focus:ring-2 focus:ring-blue-500 outline-none resize-none"
                            ...
                            <div className="flex flex-col gap-2 overflow-y-auto max-h-[600px]">
                                {variables.length > 0 ? variables.map(v => (
                                    <button 
                                        ...
                                        className="text-left px-3 py-2 bg-base-100 rounded border border-blue-200 hover:bg-blue-100 hover:border-blue-300 transition-colors text-sm font-mono text-blue-800 flex justify-between items-center group"
                                    >
                                        {v}
        ...
        <div className="space-y-6">
             <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-base-content/60">Créez et personnalisez vos documents types</p>
                </div>
                ...
                {templates.map(t => (
                    <Card key={t.id} className="group hover:shadow-md transition-shadow relative">
                        <div className="absolute top-4 right-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => setEditingTemplate(t)} className="p-1 text-gray-400 hover:text-blue-500 bg-base-100 rounded-full shadow-sm"><Edit2 size={16}/></button>
                            <button onClick={() => handleDelete(t.id)} className="p-1 text-gray-400 hover:text-red-500 bg-base-100 rounded-full shadow-sm"><Trash2 size={16}/></button>
                        </div>
                        ...
                        <div className="flex items-start gap-4 mb-4">
                            ...
                            <div>
                                <h3 className="font-bold text-base-content/90">{t.name}</h3>
                                <span className="text-xs px-2 py-1 bg-base-300 rounded-full text-base-content/70 capitalize">{types.find(type => type.id === t.type)?.label || t.type}</span>
                            </div>
                        </div>
