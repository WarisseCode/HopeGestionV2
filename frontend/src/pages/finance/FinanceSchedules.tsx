...
const FinanceSchedules: React.FC<FinanceSchedulesProps> = ({ month, year }) => {
    ...
    if (schedules.length === 0) {
        return (
            <Card className="border-none shadow-lg bg-white dark:bg-slate-800 text-center py-16">
                <Clock size={48} className="mx-auto text-gray-300 mb-4" />
                <h3 className="text-lg font-bold text-gray-700 dark:text-gray-200">Aucune échéance pour cette période</h3>
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-sm border border-gray-100 dark:border-slate-700/50">
                <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-bold text-gray-700 dark:text-gray-200">Taux de recouvrement</span>
                    ...
                    <button
                        ...
                        className={`px-4 py-2 rounded-xl text-sm font-semibold transition-all ${
                            filter === f
                                ...
                                : 'bg-gray-50 dark:bg-slate-900/50 text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:bg-slate-800/50 border border-transparent'
                        }`}
                    >
                    ...
                    {filteredSchedules.map((schedule, i) => (
                        ...
                        >
                            <Card className={`border-none shadow-sm hover:shadow-md transition-all ${
                                ...
                                'bg-white dark:bg-slate-800 border-l-4 border-l-orange-400'
                            }`}>
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 p-4">
                                        ...
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 text-xs text-gray-500 dark:text-gray-400 mt-1">
                                                {schedule.lot_reference && (
                                                    <span className="bg-gray-100 dark:bg-slate-800/50 px-2 py-0.5 rounded-md">🏠 {schedule.lot_reference}</span>
                                                )}
                                                {schedule.reference_bail && (
                                                    <span className="bg-gray-100 dark:bg-slate-800/50 px-2 py-0.5 rounded-md">📋 {schedule.reference_bail}</span>
                                                )}
                                    ...
                                    <div className="text-right flex-shrink-0">
                                        <p className="text-lg font-extrabold text-gray-900 dark:text-white">{formatCurrency(schedule.total_amount)}</p>
                                        ...
                                        {schedule.status === 'paid' ? (
                                            <div className="flex items-center gap-2">
                                                ...
                                                {schedule.quittance_url && (
                                                    <Button
                                                        variant="ghost"
                                                        className="rounded-xl px-3 shadow-sm bg-gray-100 dark:bg-slate-800/50 hover:bg-gray-200 text-gray-700 dark:text-gray-200 border-none h-10 w-10 p-0 flex items-center justify-center"
                                        ...
                                        {schedule.online_payment_status && schedule.status !== 'paid' && (
                                            <span className={`inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-bold border ${
                                                schedule.online_payment_status === 'pending'
                                                    ...
                                                    : 'bg-gray-50 dark:bg-slate-900/50 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-slate-700'
                                            }`}>
                                                <Wifi size={12} />
            ...
            >
                {selectedSchedule && (
                    <div className="space-y-5">
                        <div className="bg-gray-50 dark:bg-slate-900/50 rounded-xl p-4 space-y-2">
                            <div className="flex justify-between">
                                <span className="text-sm text-gray-500 dark:text-gray-400">Locataire</span>
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Mode de paiement</label>
                            <select
                                value={paymentMethod}
                                onChange={(e) => setPaymentMethod(e.target.value)}
                                className="w-full border border-gray-200 dark:border-slate-700 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary/30 focus:border-primary outline-none"
                            >
                        ...
                        <div>
                            <label className="block text-sm font-semibold text-gray-700 dark:text-gray-200 mb-2">Référence (optionnel)</label>
                            <input
                                type="text"
                                value={paymentRef}
