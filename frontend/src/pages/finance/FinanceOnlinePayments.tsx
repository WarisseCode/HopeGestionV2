...
const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode; bg: string }> = {
    ...
    cancelled: { label: 'Annulé', color: 'text-gray-700', icon: <XCircle size={14} />, bg: 'bg-gray-50 border-gray-200' },
    expired: { label: 'Expiré', color: 'text-gray-500', icon: <XCircle size={14} />, bg: 'bg-gray-50 border-gray-200' }
};
    ...
    return (
        <div className="space-y-6">
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    >
                        ...
                        <div className="text-2xl font-black text-gray-800">{stats.total_online}</div>
                    </motion.div>
                    <motion.div
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
            ...
            <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" onClick={loadData} className="text-gray-500">
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-gray-100">
                                    <th className="text-left py-3 px-4 text-gray-500 font-semibold text-xs">Locataire</th>
                                    <th className="text-left py-3 px-4 text-gray-500 font-semibold text-xs">Immeuble / Lot</th>
                                    <th className="text-left py-3 px-4 text-gray-500 font-semibold text-xs">Échéance</th>
                                    <th className="text-right py-3 px-4 text-gray-500 font-semibold text-xs">Montant</th>
                                    <th className="text-center py-3 px-4 text-gray-500 font-semibold text-xs">Opérateur</th>
                                    <th className="text-center py-3 px-4 text-gray-500 font-semibold text-xs">Statut</th>
                                    <th className="text-right py-3 px-4 text-gray-500 font-semibold text-xs">Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                    ...
                                    return (
                                        ...
                                        >
                                            <td className="py-3 px-4">
                                                <div className="font-semibold text-gray-800">{t.tenant_prenoms} {t.tenant_nom}</div>
                                                {t.tenant_telephone && (
                                                    <div className="text-xs text-gray-400">{t.tenant_telephone}</div>
                                                )}
                                            </td>
                                            <td className="py-3 px-4">
                                                <div className="text-gray-700">{t.building_name || '-'}</div>
