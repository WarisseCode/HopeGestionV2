...
const statusConfig: Record<string, { label: string; color: string; icon: React.ReactNode; bg: string }> = {
    ...
    cancelled: { label: 'Annulé', color: 'text-base-content/80', icon: <XCircle size={14} />, bg: 'bg-base-200 border-base-300' },
    expired: { label: 'Expiré', color: 'text-base-content/60', icon: <XCircle size={14} />, bg: 'bg-base-200 border-base-300' }
};
    ...
    return (
        <div className="space-y-6">
            ...
            {stats && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <motion.div
                        ...
                        className="bg-base-100 rounded-xl p-4 border border-base-200 shadow-sm"
                    >
                        <div className="flex items-center gap-2 text-base-content/60 text-xs font-medium mb-1">
                            <Wifi size={14} />
                            Total Transactions
                        </div>
                        <div className="text-2xl font-black text-base-content/90">{stats.total_online}</div>
                    </motion.div>
                    <motion.div
                        initial={{ opacity: 0, y: 10 }}
                        animate={{ opacity: 1, y: 0 }}
                        transition={{ delay: 0.05 }}
                        className="bg-base-100 rounded-xl p-4 border border-green-100 shadow-sm"
                    >
                    ...
                    <motion.div
                        ...
                        className="bg-base-100 rounded-xl p-4 border border-amber-100 shadow-sm"
                    >
                    ...
                    <motion.div
                        ...
                        className="bg-base-100 rounded-xl p-4 border border-red-100 shadow-sm"
                    >
            ...
            <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" onClick={loadData} className="text-base-content/60">
                    <RefreshCw size={16} className={loading ? 'animate-spin' : ''} />
                </Button>
            </div>

            {/* Transactions Table */}
            <Card className="border-none shadow-lg bg-base-100 overflow-hidden">
                {loading ? (
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="border-b border-base-200">
                                    <th className="text-left py-3 px-4 text-base-content/60 font-semibold text-xs">Locataire</th>
                                    <th className="text-left py-3 px-4 text-base-content/60 font-semibold text-xs">Immeuble / Lot</th>
                                    <th className="text-left py-3 px-4 text-base-content/60 font-semibold text-xs">Échéance</th>
                                    <th className="text-right py-3 px-4 text-base-content/60 font-semibold text-xs">Montant</th>
                                    <th className="text-center py-3 px-4 text-base-content/60 font-semibold text-xs">Opérateur</th>
                                    <th className="text-center py-3 px-4 text-base-content/60 font-semibold text-xs">Statut</th>
                                    <th className="text-right py-3 px-4 text-base-content/60 font-semibold text-xs">Date</th>
                                </tr>
                            </thead>
                            <tbody>
                                {transactions.map((t, idx) => {
                                    ...
                                    return (
                                        <motion.tr
                                            ...
                                            className="border-b border-gray-50 hover:bg-base-200/50 transition-colors"
                                        >
