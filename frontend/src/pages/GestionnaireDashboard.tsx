        ...
        <div className="flex items-center gap-3 flex-wrap">
            ...
            {managerCode && (
                <div className="relative">
                    <div 
                        className="bg-white dark:bg-slate-800 px-4 py-2 rounded-full shadow-sm border border-gray-200 dark:border-slate-700 flex items-center gap-2 cursor-pointer hover:bg-gray-50 dark:bg-slate-900/50 transition-colors group"
                        onClick={() => setShowCodeDropdown(!showCodeDropdown)}
                        title="Codes d'accès par propriétaire géré"
                    >
                        <Key size={14} className="text-primary" />
                        <span className="text-xs text-gray-500 dark:text-gray-400 font-medium uppercase">Code{managedOwners.length > 1 ? 's' : ''} Agence</span>
                    ...
                    {showCodeDropdown && (
                        <div className="absolute right-0 mt-2 w-72 bg-white dark:bg-slate-800 rounded-2xl shadow-xl border border-gray-100 dark:border-slate-700/50 z-50 overflow-hidden">
                            <div className="px-4 py-3 border-b border-gray-100 dark:border-slate-700/50 bg-gray-50 dark:bg-slate-900/50">
                                <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">Codes d'accès locataires</p>
                            </div>
                            {managedOwners.map((o) => (
                                <div key={o.owner_id} className="flex items-center justify-between px-4 py-3 hover:bg-gray-50 dark:bg-slate-900/50 transition-colors">
                                    <div>
                                        <p className="text-sm font-semibold text-gray-800 dark:text-gray-100 truncate max-w-[140px]">{o.owner_name}</p>
      ...
      <motion.div variants={itemVariants}>
        <div className="bg-gradient-to-r from-indigo-600 via-indigo-700 to-purple-700 rounded-2xl p-6 text-white shadow-xl relative overflow-hidden">
          ...
          <div className="absolute inset-0 opacity-10">
            <div className="absolute top-0 right-0 w-64 h-64 bg-white dark:bg-slate-800 rounded-full -translate-y-1/2 translate-x-1/3"></div>
            <div className="absolute bottom-0 left-0 w-48 h-48 bg-white dark:bg-slate-800 rounded-full translate-y-1/2 -translate-x-1/4"></div>
          </div>
          ...
          <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="flex gap-4 flex-wrap">
              <div className="text-center bg-white dark:bg-slate-800/10 rounded-xl px-4 py-2 backdrop-blur-sm">
       ...
       <motion.div variants={itemVariants} className="relative">
          <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
                ...
                <span className="px-2 py-0.5 rounded-full bg-gray-100 dark:bg-slate-800/50 text-xs font-medium text-gray-500 dark:text-gray-400">
          ...
          {isLoadingKpis ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {[1, 2, 3, 4].map((i) => (
                <div key={i} className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-gray-100 dark:border-slate-700/50 animate-pulse">
                  <div className="flex items-center justify-between mb-4">
        ...
        <div className="xl:col-span-2 space-y-8">
            ...
            <motion.div variants={itemVariants}>
              {isLoadingChart ? (
                ...
                <Card className="overflow-hidden border-none shadow-xl bg-white dark:bg-slate-800">
                    <div className="flex justify-between items-center mb-6 px-2">
                        <div>
                            <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">Analyse Financière</h3>
                            <p className="text-sm text-gray-500 dark:text-gray-400">Revenus vs Dépenses ({
            ...
            <motion.div variants={itemVariants}>
                ...
                {loadingWidgets ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {[1, 2].map(i => (
                             <div key={i} className="bg-gray-100 dark:bg-slate-800/50 rounded-2xl h-32 animate-pulse"></div>
                        ))}
                    </div>
                ) : featuredProperties.length > 0 ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        {featuredProperties.map(property => (
                            <div key={property.id} className="bg-white dark:bg-slate-800 rounded-2xl p-4 shadow-lg border border-gray-100 dark:border-slate-700/50 flex gap-4 hover:shadow-xl transition-all cursor-pointer group" onClick={() => navigate(`/dashboard/biens?id=${property.id}`)}>
                                <div className="w-24 h-24 rounded-xl bg-gray-200 overflow-hidden relative shrink-0">
                                    {property.image ? (
                                        ...
                                        <div className="w-full h-full flex items-center justify-center bg-gray-100 dark:bg-slate-800/50 text-gray-400">
                                            <Building2 size={32} />
                                        </div>
                                    )}
                                ...
                                <div className="flex-1 min-w-0 py-1">
                                    ...
                                    <p className="text-sm text-gray-500 dark:text-gray-400 mb-2 truncate">{property.location}</p>
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-medium bg-blue-50 text-blue-600 px-2 py-1 rounded-lg">{property.units} Lots</span>
                                        <span className="text-sm font-bold text-gray-900 dark:text-white">{property.occupancy}% Occ.</span>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <div className="p-8 text-center text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-slate-900/50 rounded-xl">
        ...
        <div className="space-y-8">
            ...
            <motion.div variants={itemVariants}>
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-slate-700/50">
                    <h3 className="font-bold text-gray-800 dark:text-gray-100 mb-4">Actions Rapides</h3>
            ...
            <motion.div variants={itemVariants}>
                 <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-slate-700/50 relative overflow-hidden">
                    <h3 className="font-bold text-gray-800 dark:text-gray-100 mb-2">Occupation</h3>
                    <div className="h-[200px] flex items-center justify-center relative z-10">
                        ...
                        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                            <span className="text-3xl font-extrabold text-gray-800 dark:text-gray-100">{stats?.tauxOccupation || 0}%</span>
            ...
            <motion.div variants={itemVariants}>
                <div className="bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg border border-gray-100 dark:border-slate-700/50">
                    <div className="flex justify-between items-center mb-6">
                        <h3 className="font-bold text-gray-800 dark:text-gray-100">Fil d'actualité</h3>
                    ...
                    {loadingWidgets ? (
                         <div className="space-y-4">
                             {[1, 2, 3].map(i => <div key={i} className="h-16 bg-gray-100 dark:bg-slate-800/50 rounded-xl animate-pulse"></div>)}
                         </div>
                    ) : activities.length > 0 ? (
                        <ActivityFeed activities={activities} />
                    ) : (
                        <p className="text-center text-gray-500 dark:text-gray-400 py-4">Aucune activité récente</p>
                    )}
                </div>
            </motion.div>
