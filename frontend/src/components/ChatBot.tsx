      ...
      <AnimatePresence>
        {(isOpen && !isMinimized) && (
          ...
          >
                  ...
                  {message.sender === 'bot' && (
                    <div className="ml-10 mt-2 w-[85%] space-y-2">
                        ...
                        <div className="flex gap-3 overflow-x-auto pb-4 pt-1 snap-x scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-transparent px-1">
                          {message.data.map((prop: any) => (
                            <div key={prop.id} className="min-w-[200px] w-[200px] bg-base-100 rounded-xl overflow-hidden shadow-sm border border-base-200 snap-center flex-shrink-0 hover:shadow-md transition-all cursor-pointer group"
                              ...
                              <div className="p-3">
                                <h4 className="font-bold text-xs truncate mb-1 text-base-content/90" title={prop.titre}>{prop.titre}</h4>
                      ...
                      {message.type === 'contact_form' && (
                         <div className="bg-base-100 p-4 rounded-xl shadow-sm border border-base-200 w-full animate-fadeIn">
                           <div className="space-y-3">
                              <div>
                                <label className="text-xs font-medium text-base-content/60 ml-1">Email</label>
                                <input type="email" placeholder="exemple@email.com" className="w-full px-3 py-2 text-sm border border-base-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 bg-base-200 transition-all" />
                              </div>
                              <div>
            ...
            {/* Input Area */}
            <div className="p-4 bg-base-100 border-t border-base-200 z-10">
                ...
                <div className="flex gap-2 overflow-x-auto pb-3 mb-1 scrollbar-hide">
                  {QUICK_SUGGESTIONS.map((sugg, i) => (
                    <button 
                      ...
                      className="whitespace-nowrap px-3 py-1.5 text-xs bg-base-200 hover:bg-base-300 text-base-content/70 border border-base-300 rounded-full transition-colors flex items-center gap-1 font-medium"
                    >
                      {sugg.label}
                    </button>
                  ))}
