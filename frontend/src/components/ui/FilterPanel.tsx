...
const FilterPanel: React.FC<FilterPanelProps> = ({
      ...
      case 'select':
        return (
          <select
            ...
            className="select select-bordered select-sm w-full bg-white dark:bg-slate-800 focus:border-primary focus:ring-1 focus:ring-primary/20"
          >
                ...
                <button
                  ...
                  className={`
                    ...
                    ${isSelected
                      ...
                      : 'bg-gray-100 dark:bg-slate-800/50 text-gray-600 dark:text-gray-300 hover:bg-gray-200'
                    }
                  `}
                >
                  {option.label}
      ...
      case 'toggle':
        return (
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-gray-600 dark:text-gray-300">{filter.placeholder || filter.label}</span>
          </label>
        );
    ...
    <div className={`${layout === 'horizontal' ? 'flex flex-wrap items-end gap-4' : 'space-y-4'}`}>
      ...
      {activeFiltersCount > 0 && (
        <Button
          ...
          className="text-gray-500 dark:text-gray-400 hover:text-error"
        >
  ...
  if (!collapsible) {
    return (
      <div className={`bg-white dark:bg-slate-800 rounded-xl p-4 border border-gray-100 dark:border-slate-700/50 ${className}`}>
        {filterContent}
      </div>
    );
  }
  ...
  return (
    <div className={`bg-white dark:bg-slate-800 rounded-xl border border-gray-100 dark:border-slate-700/50 overflow-hidden ${className}`}>
      {/* Header */}
      <button
        type="button"
        onClick={() => setIsExpanded(!isExpanded)}
        className="w-full flex items-center justify-between p-4 hover:bg-gray-50 dark:bg-slate-900/50 transition-colors"
      >
        <div className="flex items-center gap-2">
          <Filter size={18} className="text-primary" />
          <span className="font-semibold text-gray-700 dark:text-gray-200">Filtres</span>
      ...
      <AnimatePresence>
        {isExpanded && (
          ...
          >
            <div className="p-4 pt-0 border-t border-gray-100 dark:border-slate-700/50">
              {filterContent}
            </div>
          </motion.div>
        )}
