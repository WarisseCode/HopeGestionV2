...
const ChatWindow: React.FC<ChatWindowProps> = ({ contextType, contextId, recipientId, title, onClose }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-white rounded-lg shadow-lg border overflow-hidden">
            {/* Header */}
                ...
                {messages.map((msg) => {
                    ...
                    return (
                        <div key={msg.id} className={`flex flex-col ${msg.channel === 'internal' ? 'items-start' : 'items-end'}`}> 
                            <div className={`max-w-[80%] rounded-lg p-3 ${msg.channel === 'internal' ? 'bg-white border' : 'bg-blue-100'}`}>
                                <div className="text-xs font-bold mb-1 text-gray-500">{msg.sender_name} <span className="font-normal opacity-70">via {msg.channel}</span></div>
