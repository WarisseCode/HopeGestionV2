...
async function runTest() {
    try {
        ...
        const leaseRes = await pool.query('SELECT id FROM leases LIMIT 1');
        if (leaseRes.rows.length === 0) {
            // Create a dummy lease if none
             await pool.query(`INSERT INTO buildings (nom, type, adresse, ville, user_id) VALUES ('Imm Test', 'Maison', 'Cotonou', 'Cotonou', 1)`);
             const lotRes = await pool.query(`INSERT INTO lots (building_id, ref_lot, type, loyer_mensuel) VALUES ((SELECT id FROM buildings LIMIT 1), 'T1', 'Appart', 50000) RETURNING id`);
             const tRes = await pool.query(`INSERT INTO tenants (nom, prenoms, telephone_principal) VALUES ('Locataire', 'Test', '+22999999999') RETURNING id`);
             await pool.query(`INSERT INTO leases (lot_id, tenant_id, date_debut, loyer_actuel) VALUES ($1, $2, CURRENT_DATE, 50000)`, [lotRes.rows[0].id, tRes.rows[0].id]);
             console.log("⚠️  Bail de test créé.");
        }
        
        const leaseResFinal = await pool.query('SELECT id FROM leases LIMIT 1');
