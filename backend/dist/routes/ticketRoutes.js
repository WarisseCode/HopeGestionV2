...
router.post('/', authMiddleware_1.protect, async (req, res) => {
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO tickets (lot_id, titre, category, description, priorite, urgency, photos_before, requester_name, requester_phone, statut, date_creation)
             VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'Ouvert', NOW()) RETURNING *`, [lotIdValue, type, category, description, priorite, urgency || priorite, JSON.stringify(photos_before || []), requester_name, requester_phone]);
...
router.put('/:id', authMiddleware_1.protect, async (req, res) => {
    try {
        ...
        if (provider_id) {
            const result = await database_1.default.query(`UPDATE tickets SET provider_id=$1, scheduled_date=$2, cost_estimated=$3, statut=$4, updated_at=NOW()
                 WHERE id=$5 RETURNING *`, [provider_id, scheduled_date, cost_estimated, statut || 'En cours', id]);
            res.json(result.rows[0]);
        }
        else {
            const result = await database_1.default.query(`UPDATE tickets SET statut=$1, updated_at=NOW() WHERE id=$2 RETURNING *`, [statut, id]);
            res.json(result.rows[0]);
        }
    }
    catch (error) {
...
router.post('/:id/reschedule', authMiddleware_1.protect, async (req, res) => {
    try {
        const { new_date, reason } = req.body;
        const id = req.params.id;
        const result = await database_1.default.query(`UPDATE tickets SET 
                scheduled_date = $1, 
                statut = 'En attente',
...
router.post('/:id/close', authMiddleware_1.protect, async (req, res) => {
    try {
