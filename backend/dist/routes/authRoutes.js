...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        }, config_1.JWT_SECRET, { expiresIn } // Configurable expiration - cast needed for strict typing
        );
...
router.post('/verify-email', async (req, res) => {
    ...
    try {
        ...
        const expiresIn = process.env.JWT_EXPIRES_IN || '24h';
        const token = jsonwebtoken_1.default.sign({ id: user.id, role: user.role, userType: user.user_type || 'gestionnaire' }, config_1.JWT_SECRET, { expiresIn });
        res.status(200).json({
