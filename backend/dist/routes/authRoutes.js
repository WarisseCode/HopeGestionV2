 ...
 */
async function issueTokenPair(userId, role, userType) {
    const accessToken = jsonwebtoken_1.default.sign({ id: userId, role, userType }, config_1.JWT_SECRET, { expiresIn: config_1.ACCESS_TOKEN_EXPIRES_IN });
    const rawRefresh = crypto_1.default.randomBytes(40).toString('hex');
    const tokenHash = crypto_1.default.createHash('sha256').update(rawRefresh).digest('hex');
    const expiresAt = new Date(Date.now() + REFRESH_TOKEN_MS);
    await database_1.default.query(`INSERT INTO refresh_tokens (user_id, token_hash, expires_at) VALUES ($1, $2, $3)`, [userId, tokenHash, expiresAt]);
...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO users (email, password_hash, nom, user_type, role, telephone, is_verified, verification_otp, otp_expires_at) 
             VALUES ($1, $2, TRIM($3 || ' ' || $4), $5, $5, $6, false, $7, $8) RETURNING id`, [sanitizedEmail, password_hash, nom, prenoms, userType || 'gestionnaire', finalPhone, otp, otpExpiresAt]);
        // Envoyer l'email avec le code OTP
        try {
            await EmailService_1.default.sendEmail(sanitizedEmail, 'Vérification de votre compte - Hope Gestion', `Bonjour ${prenoms},\n\nMerci de vous être inscrit sur Hope Gestion.\nVotre code de vérification est : ${otp}\nIl est valide pendant 15 minutes.\n\nL'équipe Hope Gestion.`, `
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query('SELECT id, password_hash, role, user_type, statut, is_verified FROM users WHERE email = $1', [sanitizedEmail]);
        ...
        const passwordMatch = await bcrypt_1.default.compare(password, utilisateur.password_hash);
        ...
        const { accessToken, refreshToken } = await issueTokenPair(utilisateur.id, utilisateur.role, utilisateur.user_type || 'gestionnaire');
...
router.post('/verify-email', async (req, res) => {
    ...
    try {
        ...
        const sanitizedEmail = email.trim().toLowerCase();
        const result = await database_1.default.query('SELECT id, verification_otp, otp_expires_at, is_verified, role, user_type FROM users WHERE email = $1', [sanitizedEmail]);
        if (result.rows.length === 0) {
        ...
        await database_1.default.query('UPDATE users SET is_verified = true, verification_otp = NULL, otp_expires_at = NULL WHERE id = $1', [user.id]);
        // Connexion automatique après vérification
        const { accessToken, refreshToken } = await issueTokenPair(user.id, user.role, user.user_type || 'gestionnaire');
        res.status(200).json({
...
router.post('/resend-otp', async (req, res) => {
    ...
    try {
        ...
        const sanitizedEmail = email.trim().toLowerCase();
        const result = await database_1.default.query('SELECT id, nom, is_verified FROM users WHERE email = $1', [sanitizedEmail]);
        if (result.rows.length === 0) {
        ...
        await database_1.default.query('UPDATE users SET verification_otp = $1, otp_expires_at = $2 WHERE id = $3', [newOtp, newExpiresAt, user.id]);
        // Envoyer l'email
        await EmailService_1.default.sendEmail(sanitizedEmail, 'Nouveau code de vérification - Hope Gestion', `Bonjour ${user.nom},\n\nVotre nouveau code de vérification est : ${newOtp}\n\nL'équipe Hope Gestion.`, `
            <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
                <div style="background-color: #2c3e50; padding: 20px; text-align: center;">
...
router.post('/refresh', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query(`SELECT rt.*, u.role, u.user_type
             FROM refresh_tokens rt
             JOIN users u ON rt.user_id = u.id
             WHERE rt.token_hash = $1
               AND rt.revoked_at IS NULL
               AND rt.expires_at > NOW()`, [tokenHash]);
        if (result.rows.length === 0) {
            return res.status(401).json({ message: 'Refresh token invalide ou expiré.' });
        }
        const stored = result.rows[0];
        // Révoquer l'ancien refresh token (rotation)
        await database_1.default.query(`UPDATE refresh_tokens SET revoked_at = NOW() WHERE id = $1`, [stored.id]);
        // Émettre une nouvelle paire de tokens
        const { accessToken, refreshToken: newRefreshToken } = await issueTokenPair(stored.user_id, stored.role, stored.user_type || 'gestionnaire');
...
router.post('/logout', async (req, res) => {
    ...
    if (refreshToken) {
        try {
            const tokenHash = crypto_1.default.createHash('sha256').update(refreshToken).digest('hex');
            await database_1.default.query(`UPDATE refresh_tokens SET revoked_at = NOW() WHERE token_hash = $1`, [tokenHash]);
        }
        catch (err) {
...
router.get('/profile', verifyToken, async (req, res) => {
    try {
        ...
        if (user.role === 'admin') {
            ...
            const matrixResult = await database_1.default.query(`SELECT module, can_read, can_write, can_validate, can_delete FROM permission_matrix WHERE role = $1`, [user.role]);
            ...
            const delegationResult = await database_1.default.query(`SELECT can_view_finances, can_edit_properties, can_manage_tenants,
...
router.put('/profile', verifyToken, async (req, res) => {
    ...
    try {
        ...
        await database_1.default.query(`UPDATE users 
             ...
             WHERE id = $6`, [fullName, email, telephone, JSON.stringify(preferences), photo_url, userId]);
        // Log
        await AuditService_1.AuditService.log({
...
router.post('/change-password', verifyToken, async (req, res) => {
    ...
    try {
        ...
        const match = await bcrypt_1.default.compare(currentPassword, user.password_hash);
        if (!match) {
...
router.post('/invite-user', verifyToken, async (req, res) => {
...
router.post('/accept-invite', async (req, res) => {
    ...
    try {
        ...
        // 1. Vérifier le token
        const inviteRes = await database_1.default.query(`SELECT * FROM user_invitations WHERE token = $1 AND expires_at > NOW() AND used_at IS NULL`, [token]);
...
router.post('/create-guest', verifyToken, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            const insertRes = await client.query(`INSERT INTO users (email, password_hash, nom, user_type, role, telephone, statut, access_key, access_key_expires_at, is_guest, agency_id) 
                 VALUES ($1, $2, TRIM($3 || ' ' || $4), 'guest', $5, $6, 'actif', $7, $8, true, $9) 
                 RETURNING id`, [dummyEmail, dummyHash, nom, prenom || '', guestRole, telephone, accessKey, expiresAt, agencyId]);
            ...
            const issuerAssignmentRes = await client.query(`SELECT owner_id FROM owner_user WHERE user_id = $1 AND is_active = true LIMIT 1`, [issuerId]);
            ...
            if (role && !permissions) {
                ...
                const permMatrixRes = await client.query(`SELECT module, can_read, can_write, can_delete, can_validate 
                ...
                for (const row of permMatrixRes.rows) {
                    ...
                    if (module === 'finance') {
                        if (row.can_read)
                            userPermissions.can_view_finances = true;
                        if (row.can_validate)
                            userPermissions.can_validate_payments = true;
                    }
                    // Biens (Properties) module
                    if (module === 'biens') {
                        if (row.can_write)
                            userPermissions.can_edit_properties = true;
                    }
                    // Locataires (Tenants) module
                    if (module === 'locataires') {
                        if (row.can_write)
                            userPermissions.can_manage_tenants = true;
                    }
                    // Owners/Contracts module
                    if (module === 'owners' || module === 'contrats') {
                        if (row.can_write)
                            userPermissions.can_manage_contracts = true;
                    }
                    // Users module
                    if (module === 'users') {
                        if (row.can_write)
                            userPermissions.can_manage_users = true;
                    }
...
router.post('/login-with-key', async (req, res) => {
    ...
    try {
        if (!accessKey)
            return res.status(400).json({ message: 'Clé d\'accès requise.' });
        // 1. Find the guest user by access key
        const userResult = await database_1.default.query(`SELECT id, nom, role, access_key_expires_at, is_guest, statut 
             FROM users 
        ...
        const assignmentResult = await database_1.default.query(`SELECT ou.owner_id, ou.can_view_finances, ou.can_edit_properties, 
...
router.post('/forgot-password', async (req, res) => {
    ...
    try {
        ...
        const userResult = await database_1.default.query('SELECT id, nom, email FROM users WHERE email = $1', [sanitizedEmail]);
        ...
        await database_1.default.query('DELETE FROM password_reset_tokens WHERE user_id = $1', [user.id]);
        ...
        await database_1.default.query(`INSERT INTO password_reset_tokens (user_id, token_hash, expires_at, ip_address, user_agent)
             VALUES ($1, $2, $3, $4, $5)`, [user.id, tokenHash, expiresAt, req.ip || 'unknown', req.headers['user-agent'] || 'unknown']);
        // 8. Send email with plain token (only in email, never stored)
        const emailSent = await EmailService_1.default.sendPasswordResetEmail(user.email, resetToken, user.nom, req.ip || 'unknown');
        if (!emailSent) {
...
router.post('/reset-password', async (req, res) => {
    ...
    try {
        ...
        // 8. Get user details
        const userResult = await database_1.default.query('SELECT id, email, nom FROM users WHERE id = $1', [resetRecord.user_id]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ message: 'Utilisateur introuvable.' });
        }
        const user = userResult.rows[0];
        ...
        await database_1.default.query('UPDATE users SET password_hash = $1 WHERE id = $2', [newPasswordHash, user.id]);
        // 11. Mark token as used
        await database_1.default.query('UPDATE password_reset_tokens SET used_at = CURRENT_TIMESTAMP WHERE token_hash = $1', [tokenHash]);
        // 12. Send confirmation email
        await EmailService_1.default.sendPasswordResetConfirmation(user.email, user.nom, req.ip || 'unknown');
        // 13. Audit log
        await AuditService_1.AuditService.log({
            userId: user.id.toString(),
...
router.get('/validate-reset-token/:token', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query(`SELECT expires_at, used_at FROM password_reset_tokens WHERE token_hash = $1`, [tokenHash]);
        if (result.rows.length === 0) {
