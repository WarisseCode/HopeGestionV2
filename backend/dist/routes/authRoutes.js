...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO users (email, password_hash, nom, user_type, role, telephone) 
             VALUES ($1, $2, TRIM($3 || ' ' || $4), $5, $5, $6) RETURNING id`, [email, password_hash, nom, prenoms, userType || 'gestionnaire', finalPhone]);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query('SELECT id, password_hash, role, statut FROM users WHERE email = $1', [email]);
        ...
        const match = await bcrypt_1.default.compare(password, utilisateur.password_hash);
        if (!match) {
            return res.status(401).json({
        ...
        const token = jsonwebtoken_1.default.sign({ id: utilisateur.id, role: utilisateur.role }, JWT_SECRET, { expiresIn: '1d' } // Le jeton expire après 1 jour
        );
        // 4. Log Audit
        try {
            await AuditService_1.AuditService.log({
