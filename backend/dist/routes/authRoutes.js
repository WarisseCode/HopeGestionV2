...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`INSERT INTO users (email, password_hash, nom, user_type, telephone) 
             VALUES ($1, $2, TRIM($3 || ' ' || $4), $5, $6) RETURNING id`, [email, password_hash, nom, prenoms, userType || 'gestionnaire', telephone]);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query('SELECT id, password_hash, role, statut FROM users WHERE email = $1', [email]);
