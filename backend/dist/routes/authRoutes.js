...
router.post('/register', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query(`INSERT INTO users (email, password_hash, nom, user_type, role, telephone) 
             VALUES ($1, $2, TRIM($3 || ' ' || $4), $5, $5, $6) RETURNING id`, [sanitizedEmail, password_hash, nom, prenoms, userType || 'gestionnaire', finalPhone]);
...
router.post('/login', async (req, res) => {
    ...
    try {
        ...
        const result = await database_1.default.query('SELECT id, password_hash, role, user_type, statut FROM users WHERE email = $1', [sanitizedEmail]);
...
router.post('/invite-user', verifyToken, async (req, res) => {
    ...
    try {
        ...
        try {
            ...
            await client.query(`INSERT INTO user_invitations (token, email, role, issuer_id, permissions, expires_at)
                 VALUES ($1, $2, $3, $4, $5, $6)`, [token, email || telephone, role, issuerId, { userId }, expiresAt] // Storing userId in permissions json as a link hack or use a generic json
            );
