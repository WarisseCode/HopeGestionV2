...
router.put('/:id', authMiddleware_1.protect, async (req, res) => {
    try {
        ...
        // Handle closing
        if (status === 'done') {
            await database_1.default.query(`UPDATE tasks SET status='done', completed_at=NOW(), updated_at=NOW() WHERE id=$1`, [id]);
        }
        else {
            // General update
        ...
        if (comment) {
            await database_1.default.query(`INSERT INTO messages (sender_id, context_type, context_id, content, channel)
                 VALUES ($1, 'task', $2, $3, 'internal')`, [req.user.id, id, comment]);
        }
