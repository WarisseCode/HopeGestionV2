...
const mobileMoneyService_1 = require("../services/mobileMoneyService");
...
router.post('/pay', tenantGuard_1.tenantGuard, async (req, res) => {
    ...
    try {
        ...
        const txResult = await dbClient.query(`INSERT INTO mobile_money_transactions 
            (amount, phone_number, operator, status, transaction_type, external_reference)
            VALUES ($1, $2, $3, 'pending', 'collection', $4)
            RETURNING id, transaction_id`, [amount, phoneNumber, operator, description]);
        const internalId = txResult.rows[0].id;
        // 2. Appeler le service MoMo (API externe, pas de dbClient)
        const response = await mobileMoneyService_1.mobileMoneyService.requestPayment({
            amount,
            phoneNumber,
            operator: operator
        });
        // 3. Mettre à jour la transaction avec le résultat
        await dbClient.query(`UPDATE mobile_money_transactions 
            SET status = $1, transaction_id = $2, metadata = $3 
            WHERE id = $4`, [response.status, response.transactionId, JSON.stringify(response), internalId]);
        ...
        if (response.status === 'success' && leaseId) {
            ...
            if (leaseRes.rows.length > 0) {
                const payResult = await dbClient.query(`INSERT INTO payments 
                     ...
                     RETURNING id`, [leaseId, amount, operator, response.transactionId, strictOwnerId]);
...
router.get('/transactions', tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    try {
...
router.get('/configs', tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Non authentifié" });
        const configs = await mobileMoneyService_1.mobileMoneyService.getConfigs(dbClient, req.userId);
        res.json(configs);
    }
    catch (error) {
...
router.post('/configs', tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Non authentifié" });
        const config = await mobileMoneyService_1.mobileMoneyService.addConfig(dbClient, { ...req.body, userId: req.userId });
        res.json(config);
    }
    catch (error) {
...
router.put('/configs/:id', tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Non authentifié" });
        ...
        const config = await mobileMoneyService_1.mobileMoneyService.updateConfig(dbClient, parseInt(id), req.userId, req.body);
        res.json(config);
    }
    catch (error) {
...
router.delete('/configs/:id', tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Non authentifié" });
        ...
        if (!id)
            return res.status(400).json({ message: "ID manquant" });
        await mobileMoneyService_1.mobileMoneyService.deleteConfig(dbClient, parseInt(id), req.userId);
...
router.patch('/configs/:id/toggle', tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    try {
        if (!req.userId)
            return res.status(401).json({ message: "Non authentifié" });
        ...
        if (!id)
            return res.status(400).json({ message: "ID manquant" });
        const config = await mobileMoneyService_1.mobileMoneyService.toggleConfigStatus(dbClient, parseInt(id), req.userId);
        res.json(config);
    }
    catch (error) {
