...
router.post('/pay', async (req, res) => {
    ...
    try {
        ...
        const txResult = await index_1.pool.query(`INSERT INTO mobile_money_transactions 
        ...
        await index_1.pool.query(`UPDATE mobile_money_transactions 
        ...
        if (response.status === 'success' && leaseId) {
            ...
            const leaseRes = await index_1.pool.query('SELECT owner_id FROM leases WHERE id = $1', [leaseId]);
            let ownerId = null;
            if (leaseRes.rows.length > 0)
                ownerId = leaseRes.rows[0].owner_id;
            const payResult = await index_1.pool.query(`INSERT INTO payments 
                 ...
                 RETURNING id`, [leaseId, amount, operator, response.transactionId, ownerId]);
            // Lier transaction technique au paiement métier
            await index_1.pool.query('UPDATE mobile_money_transactions SET payment_id = $1 WHERE id = $2', [payResult.rows[0].id, internalId]);
        }
        res.json({
...
router.get('/transactions', async (req, res) => {
    try {
        const result = await index_1.pool.query('SELECT * FROM mobile_money_transactions ORDER BY created_at DESC LIMIT 50');
