...
router.get('/:id', authMiddleware_1.protect, ownerIsolation_1.checkOwnerAccess, async (req, res) => {
    try {
        const ownerId = req.params.id;
        const ownersResult = await database_1.default.query(`SELECT * FROM owners WHERE id = $1 AND is_active = TRUE`, [ownerId]);
        if (ownersResult.rows.length === 0) {
            return res.status(404).json({ success: false, message: 'Propriétaire non trouvé' });
        }
        const usersResult = await database_1.default.query(`SELECT u.id, u.nom, u.email, u.role, ou.role as assignment_role,
...
router.post('/', authMiddleware_1.protect, async (req, res) => {
    try {
        ...
        const existingResult = await database_1.default.query('SELECT id FROM owners WHERE phone = $1', [phone]);
        ...
        const userId = req.userId;
        await database_1.default.query(`INSERT INTO owner_user (owner_id, user_id, role, start_date, is_active)
             VALUES ($1, $2, 'owner', CURRENT_DATE, TRUE)`, [ownerId, userId]);
...
router.put('/:id', authMiddleware_1.protect, ownerIsolation_1.checkOwnerAccess, async (req, res) => {
    try {
        const ownerId = req.params.id;
        const { type, name, first_name, phone, phone_secondary, email, address, city, country, id_number, photo, mobile_money_number, management_mode } = req.body;
        await database_1.default.query(`UPDATE owners SET
             ...
             WHERE id = $14`, [type, name, first_name, phone, phone_secondary, email, address, city, country, id_number, photo, mobile_money_number, management_mode, ownerId]);
...
router.delete('/:id', authMiddleware_1.protect, ownerIsolation_1.checkOwnerAccess, async (req, res) => {
    try {
        await database_1.default.query('UPDATE owners SET is_active = FALSE WHERE id = $1', [req.params.id]);
...
router.post('/:id/users', authMiddleware_1.protect, ownerIsolation_1.checkOwnerAccess, async (req, res) => {
    try {
        const ownerId = req.params.id;
        const { user_id, role, can_view_finances, can_edit_properties, can_manage_tenants, can_manage_contracts, can_validate_payments, start_date, end_date } = req.body;
        const usersResult = await database_1.default.query('SELECT id FROM users WHERE id = $1', [user_id]);
...
router.delete('/:id/users/:userId', authMiddleware_1.protect, ownerIsolation_1.checkOwnerAccess, async (req, res) => {
    try {
        const { id: ownerId, userId } = req.params;
        await database_1.default.query('UPDATE owner_user SET is_active = FALSE, end_date = CURRENT_DATE WHERE owner_id = $1 AND user_id = $2', [ownerId, userId]);
...
router.get('/:id/properties', authMiddleware_1.protect, ownerIsolation_1.checkOwnerAccess, async (req, res) => {
    try {
