...
exports.checkAdminInvite = checkAdminInvite;
// backend/routes/adminRoutes.ts
const express_1 = require("express");
const database_1 = __importDefault(require("../db/database")); // Fix circular dependency
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const [usersRes, activeRes, agenciesRes, propertiesRes, lotsRes, tenantsRes] = await Promise.all([
            ...
            database_1.default.query("SELECT COUNT(*) FROM users WHERE statut = 'actif'"),
            database_1.default.query('SELECT COUNT(*) FROM owners'),
            database_1.default.query('SELECT COUNT(*) FROM buildings'),
            database_1.default.query('SELECT COUNT(*) FROM lots'),
            database_1.default.query('SELECT COUNT(*) FROM tenants'),
        ]);
        // Revenue — safe query with fallback
        let revenue = 0;
        let paymentsCount = 0;
        try {
            const revRes = await database_1.default.query("SELECT COALESCE(SUM(montant),0) as total, COUNT(*) as cnt FROM payments");
        ...
        try {
            const trendRes = await database_1.default.query(`
                SELECT 
...
router.get('/stats/growth', async (req, res) => {
    try {
        ...
        const usersGrowth = await database_1.default.query(`
            SELECT 
        ...
        let paymentsGrowth = [];
        try {
            const payRes = await database_1.default.query(`
                SELECT 
...
function formatAction(action, module) {
    const actions = {
...
function getActivityType(action) {
    if (action.includes('FAIL') || action.includes('ERROR') || action.includes('DELETE'))
        return 'error';
    if (action.includes('WARNING') || action.includes('SUSPEND'))
        return 'warning';
    if (action.includes('SUCCESS') || action.includes('CREATED') || action.includes('VALIDATED'))
        return 'success';
    return 'info';
}
// ==============================================
...
router.get('/users', async (req, res) => {
    try {
        ...
        const countRes = await database_1.default.query('SELECT COUNT(*) FROM users');
        res.json({
            users: result.rows,
...
router.get('/users/:id', async (req, res) => {
    ...
    try {
        ...
        let ownerData = null;
        try {
            const ownerRes = await database_1.default.query(`
            ...
            if (ownerRes.rows.length > 0)
                ownerData = ownerRes.rows;
        }
        catch (e) { }
        // Recent activity
        let recentActivity = [];
        try {
            const actRes = await database_1.default.query(`
...
router.post('/users/:id/action', async (req, res) => {
    ...
    try {
        if (action === 'promote') {
            await database_1.default.query("UPDATE users SET role = 'admin' WHERE id = $1", [id]);
            res.json({ message: 'Utilisateur promu Admin avec succès.' });
        }
        else if (action === 'suspend') {
            await database_1.default.query("UPDATE users SET statut = 'inactif' WHERE id = $1", [id]);
            res.json({ message: 'Compte utilisateur suspendu.' });
        }
        else if (action === 'reactivate') {
            await database_1.default.query("UPDATE users SET statut = 'actif' WHERE id = $1", [id]);
            ...
            await database_1.default.query("UPDATE users SET is_verified = true WHERE id = $1", [id]);
            res.json({ message: 'Compte vérifié manuellement.' });
        }
        else if (action === 'unverify') {
            await database_1.default.query("UPDATE users SET is_verified = false WHERE id = $1", [id]);
            res.json({ message: 'Vérification du compte révoquée.' });
        }
        else if (action === 'change_role') {
            if (!value)
                return res.status(400).json({ message: 'Nouveau rôle requis.' });
            ...
            await database_1.default.query("UPDATE users SET role = $1 WHERE id = $2", [value, id]);
...
router.get('/finances', async (req, res) => {
    try {
        ...
        const volRes = await database_1.default.query(`
            SELECT 
        ...
        const txRes = await database_1.default.query(`
...
router.get('/subscriptions', async (req, res) => {
    try {
        // Get all plans
        const plansRes = await database_1.default.query('SELECT * FROM subscription_plans ORDER BY price ASC');
        // Get active subscriptions with user info
        const subsRes = await database_1.default.query(`
            SELECT 
        ...
        `);
        // Count per plan
        const countPerPlan = await database_1.default.query(`
...
router.post('/subscriptions/assign', async (req, res) => {
    ...
    try {
        ...
        const planRes = await database_1.default.query('SELECT * FROM subscription_plans WHERE id = $1', [planId]);
        ...
        await database_1.default.query("UPDATE user_subscriptions SET status = 'cancelled', cancelled_at = NOW() WHERE user_id = $1 AND status = 'active'", [userId]);
        // Create new subscription
        await database_1.default.query(`INSERT INTO user_subscriptions (user_id, plan_id, status, starts_at, expires_at) 
             VALUES ($1, $2, 'active', NOW(), $3)`, [userId, planId, expiresAt]);
...
router.post('/subscriptions/:id/cancel', async (req, res) => {
    const { id } = req.params;
    try {
        await database_1.default.query("UPDATE user_subscriptions SET status = 'cancelled', cancelled_at = NOW() WHERE id = $1", [id]);
...
router.post('/invite', async (req, res) => {
    ...
    try {
        ...
        const existingInvite = await database_1.default.query('SELECT id FROM admin_invitations WHERE email = $1 AND used_at IS NULL', [email]);
        if (existingInvite.rows.length > 0) {
            return res.status(400).json({ message: 'Une invitation existe déjà pour cet email.' });
        }
        const inviteCode = generateInviteCode();
        const expiresAt = new Date(Date.now() + expiresInHours * 60 * 60 * 1000);
        await database_1.default.query(`INSERT INTO admin_invitations (email, invite_code, invited_by, expires_at)
             VALUES ($1, $2, $3, $4)`, [email, inviteCode, req.userId, expiresAt]);
...
router.delete('/invites/:id', async (req, res) => {
    const { id } = req.params;
    try {
        await database_1.default.query('DELETE FROM admin_invitations WHERE id = $1', [id]);
...
async function acceptAdminInvite(req, res) {
    ...
    try {
        // Validate invitation
        const inviteResult = await database_1.default.query(`SELECT * FROM admin_invitations 
             WHERE invite_code = $1 AND email = $2 AND used_at IS NULL AND expires_at > NOW()`, [code, email]);
        ...
        // Create admin user
        const userResult = await database_1.default.query(`INSERT INTO users (email, password_hash, nom, role, user_type, statut, created_at)
             VALUES ($1, $2, $3, 'admin', 'admin', 'actif', NOW())
             RETURNING id, email, nom, role`, [email, hashedPassword, nom || 'Admin']);
        // Mark invitation as used
        await database_1.default.query('UPDATE admin_invitations SET used_at = NOW() WHERE id = $1', [invite.id]);
        res.json({
...
async function checkAdminInvite(req, res) {
    const { code } = req.query;
    if (!code) {
        return res.status(400).json({ valid: false, message: 'Code requis.' });
    }
    try {
        const result = await database_1.default.query(`SELECT email, expires_at FROM admin_invitations 
             WHERE invite_code = $1 AND used_at IS NULL AND expires_at > NOW()`, [code]);
