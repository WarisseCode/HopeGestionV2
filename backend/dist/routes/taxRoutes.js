...
router.get('/settings', permissionMiddleware_1.default.canRead('finances'), tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    const ownerId = req.resolvedOwnerId;
    try {
        const result = await dbClient.query('SELECT * FROM tax_settings WHERE owner_id = $1', [ownerId]);
...
router.post('/settings', permissionMiddleware_1.default.canWrite('finances'), tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    const ownerId = req.resolvedOwnerId;
    try {
...
// [SÉCURITÉ] ownerId résolu via tenantGuard — jamais depuis req.params
router.get('/report/:year', permissionMiddleware_1.default.canRead('finances'), tenantGuard_1.tenantGuard, async (req, res) => {
    const dbClient = req.dbClient;
    const ownerId = req.resolvedOwnerId;
    try {
