...
router.get('/stats/proprietaire', async (req, res) => {
    ...
    try {
        ...
        const buildingsResult = await pool.query('SELECT COUNT(*) FROM buildings WHERE owner_id = $1', [ownerId]);
        const totalBiens = parseInt(buildingsResult.rows[0].count, 10);
        // Total des lots de ce propriétaire
        const lotsResult = await pool.query('SELECT COUNT(*) FROM lots WHERE owner_id = $1', [ownerId]);
        const totalLots = parseInt(lotsResult.rows[0].count, 10);
        // Lots occupés
        const occupiedResult = await pool.query("SELECT COUNT(*) FROM lots WHERE owner_id = $1 AND statut = 'occupe'", [ownerId]);
        const lotsOccupes = parseInt(occupiedResult.rows[0].count, 10);
        // Taux d'occupation
        const tauxOccupation = totalLots > 0 ? Math.round((lotsOccupes / totalLots) * 100) : 0;
...
router.get('/stats/locataire', async (req, res) => {
