...
const fedapayService_1 = require("../services/fedapayService");
// GET /api/subscriptions/plans - Liste des offres disponibles
router.get('/plans', async (req, res) => {
    try {
        const result = await index_1.pool.query(`SELECT id, name, display_name, price, duration_months, max_properties, max_tenants, features 
...
router.get('/status', authMiddleware_1.protect, async (req, res) => {
    try {
        ...
        const subResult = await index_1.pool.query(`SELECT s.*, p.name as plan_name, p.display_name, p.max_properties, p.max_tenants, p.features
...
router.post('/subscribe', authMiddleware_1.protect, async (req, res) => {
    const context = 'SUBSCRIBE';
    try {
        const userId = req.userId;
        const { plan_id, phone_number, operator } = req.body;
        fedapayService_1.fedapayLogger.info(context, 'Subscription request received', { userId, plan_id, operator });
        // Validate inputs
        ...
        // Get plan details
        const planResult = await index_1.pool.query(`SELECT * FROM plans WHERE id = $1`, [plan_id]);
        ...
        if (plan.price === 0 || plan.name === 'free') {
            fedapayService_1.fedapayLogger.info(context, 'Activating free plan directly', { userId });
            await index_1.pool.query(`INSERT INTO subscriptions (user_id, plan_id, status, start_date, end_date)
                 VALUES ($1, $2, 'active', NOW(), NULL)
                 ON CONFLICT DO NOTHING`, [userId, plan_id]);
            await index_1.pool.query(`UPDATE users SET current_plan_id = $1 WHERE id = $2`, [plan_id, userId]);
            return res.json({
                success: true,
        ...
        const userResult = await index_1.pool.query(`SELECT id, nom, email, telephone FROM users WHERE id = $1`, [userId]);
        const user = userResult.rows[0];
        // 1. Create Pending Subscription
        const subResult = await index_1.pool.query(`INSERT INTO subscriptions (user_id, plan_id, status, start_date, end_date)
             VALUES ($1, $2, 'pending_payment', NOW(), NULL)
             RETURNING id`, [userId, plan_id]);
        const subscriptionId = subResult.rows[0].id;
        // 2. Record Payment Attempt (pending)
        const paymentResult = await index_1.pool.query(`INSERT INTO subscription_payments (subscription_id, user_id, plan_id, amount, operator, phone_number, status)
             VALUES ($1, $2, $3, $4, $5, $6, 'pending')
             RETURNING id`, [subscriptionId, userId, plan_id, plan.price, operator.toLowerCase(), phone_number]);
        const paymentId = paymentResult.rows[0].id;
        fedapayService_1.fedapayLogger.info(context, 'Created pending subscription and payment', { subscriptionId, paymentId });
        // 3. Create FedaPay Transaction
        const fedaResult = await fedapayService_1.fedapayService.createPaymentTransaction({
            userId: userId,
        ...
        if (!fedaResult.success || !fedaResult.paymentUrl) {
            fedapayService_1.fedapayLogger.error(context, 'FedaPay transaction creation failed', fedaResult);
            // Mark as failed
            await index_1.pool.query(`UPDATE subscriptions SET status = 'failed' WHERE id = $1`, [subscriptionId]);
            await index_1.pool.query(`UPDATE subscription_payments SET status = 'failed' WHERE id = $1`, [paymentId]);
        ...
        await index_1.pool.query(`UPDATE subscription_payments SET transaction_reference = $1 WHERE id = $2`, [fedaResult.transactionId, paymentId]);
        fedapayService_1.fedapayLogger.info(context, '✅ FedaPay transaction created successfully', {
        ...
        fedapayService_1.fedapayLogger.error(context, 'Subscription error', error);
...
router.get('/history', authMiddleware_1.protect, async (req, res) => {
    try {
        const userId = req.userId;
        const result = await index_1.pool.query(`SELECT sp.*, p.name as plan_name, p.display_name
...
router.get('/check-payment/:transactionId', authMiddleware_1.protect, async (req, res) => {
    ...
    try {
        ...
        const paymentCheck = await index_1.pool.query(`SELECT id, status FROM subscription_payments 
             WHERE transaction_reference = $1`, [transactionId]);
        ...
        fedapayService_1.fedapayLogger.info(context, '⚠️ Manual activation triggered (Webhook fallback)', { transactionId });
        const client = await index_1.pool.connect();
        try {
            await client.query('BEGIN');
            // Get subscription ID linked to this payment
            const paymentRes = await client.query(`SELECT subscription_id, user_id, plan_id FROM subscription_payments 
                     WHERE transaction_reference = $1`, [transactionId]);
            ...
            await client.query(`UPDATE subscriptions 
                     SET status = 'active', 
                         ...
                         end_date = $2, 
                         updated_at = NOW()
                     WHERE id = $3`, [startDate, endDate, subscription_id]);
            // Update Payment status
            await client.query(`UPDATE subscription_payments 
                     SET status = 'approved', 
                         payment_date = NOW()
                     WHERE transaction_reference = $1`, [transactionId]);
            // Update User Profile
            await client.query(`UPDATE users SET current_plan_id = $1 WHERE id = $2`, [paymentPlanId, userId]);
            await client.query('COMMIT');
            fedapayService_1.fedapayLogger.info(context, '✅ Subscription activated manually', { userId, planId: paymentPlanId });
            return res.json({
                status: 'approved',
