    ...
    static async checkLatePayments(force = false) {
        ...
        try {
            ...
            for (const lease of result.rows) {
                ...
                const exists = await client.query(`SELECT id FROM notifications 
                     WHERE user_id = $1 AND title = $2 
                     AND created_at::date = CURRENT_DATE`, [userId, title]);
                if (exists.rowCount === 0) {
                    await notificationService_1.NotificationService.send(userId, title, message, 'warning');
    ...
    static async checkLeaseExpirations() {
        ...
        try {
            ...
            for (const lease of result.rows) {
                ...
                await notificationService_1.NotificationService.send(userId, title, message, 'info');
