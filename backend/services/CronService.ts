            ...
            for (const task of overdueTasks.rows) {
                ...
                if (task.assigned_to) {
                    await NotificationService.send(
                        task.assigned_to,
                        'Tâche en retard',
                        `La tâche "${task.title}" était due le ${dueDate} et n'est pas encore terminée.`,
                        'warning',
                        'task_overdue'
                    );
                }
                ...
                if (task.created_by && task.created_by !== task.assigned_to) {
                    await NotificationService.send(
                        task.created_by,
                        'Tâche en retard',
                        `La tâche "${task.title}" assignée à ${task.assigned_name || 'un collaborateur'} était due le ${dueDate}.`,
                        'warning',
                        'task_overdue'
                    );
                }
    ...
    static async checkInterventionAlerts() {
        try {
            ...
            const urgentResult = await pool.query(
                `SELECT t.id, t.titre, t.date_creation,
            ...
            for (const ticket of urgentResult.rows) {
                if (ticket.gestionnaire_id) {
                    await NotificationService.send(
                        ...
                        'Ticket urgent non traité',
                        `Le ticket #${ticket.id} "${ticket.titre}" (${ticket.building_name}) est ouvert depuis plus de 24h.`,
                        'error',
                        'ticket_urgent'
                    );
                }
            ...
            const overdueResult = await pool.query(
                `SELECT t.id, t.titre, t.scheduled_date,
            ...
            for (const ticket of overdueResult.rows) {
                if (ticket.gestionnaire_id) {
                    ...
                    await NotificationService.send(
                        ...
                        `L'intervention #${ticket.id} "${ticket.titre}" prévue le ${scheduled} chez ${ticket.provider_name || 'prestataire inconnu'} n'est pas clôturée.`,
                        'warning',
            ...
            for (const setting of settings) {
                ...
                if (setting.event_type === 'contract_end') {
                    ...
                    for (const lease of leases.rows) {
                        await NotificationService.send(
                            setting.user_id,
                            'Fin de bail imminente',
                            `Le bail de ${lease.nom} ${lease.prenoms} arrive à échéance le ${targetDateStr}.`,
                            'warning',
                            'contract_end'
                        );
                    }
                    ...
                    for (const evt of events.rows) {
                        await NotificationService.send(
                            setting.user_id,
                            'Rappel événement',
                            `Rappel : "${evt.title}" est prévu le ${targetDateStr}.`,
                            'info',
                            'custom_event'
                        );
                    }
