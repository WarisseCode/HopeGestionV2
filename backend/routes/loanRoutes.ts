...
router.put('/:id/close', permissions.canWrite('finances'), async (req: AuthenticatedRequest, res: Response) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        ...
        if (loanRes.rows.length === 0) return res.status(404).json({ message: 'Prêt non trouvé' });
        if (loanRes.rows[0].status === 'paid') return res.status(400).json({ message: 'Prêt déjà clôturé' });
...
router.put('/:id/installment/:paymentId/pay', permissions.canWrite('finances'), async (req: AuthenticatedRequest, res: Response) => {
    const client = await pool.connect();
    try {
        await client.query('BEGIN');
        ...
        if (installmentRes.rows.length === 0) return res.status(404).json({ message: 'Échéance non trouvée' });
        if (installmentRes.rows[0].status === 'paid') return res.status(400).json({ message: 'Échéance déjà payée' });
        ...
        const remainingRes = await client.query(
            'SELECT COUNT(*) as remaining FROM loan_payments WHERE loan_id = $1 AND status != $2',
            [id, 'paid']
        );
