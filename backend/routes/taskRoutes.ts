...
const taskCreateRules = [
    body('title').trim().notEmpty().withMessage('Le titre est obligatoire').isLength({ max: 255 }).withMessage('Titre trop long (max 255 caractères)'),
    body('priority').optional().isIn(['low', 'medium', 'high']).withMessage('Priorité invalide (low, medium, high)'),
    body('due_date').optional({ nullable: true }).isISO8601().withMessage('Date invalide (format ISO 8601 attendu)'),
    body('assigned_to').optional({ nullable: true }).isInt({ min: 1 }).withMessage('ID assigné invalide'),
];

const taskUpdateRules = [
    body('status').optional().isIn(['todo', 'in_progress', 'done']).withMessage('Statut invalide (todo, in_progress, done)'),
...
router.get('/', async (req: any, res) => {
    try {
        ...
        if (status)   { where += ` AND t.status = $${paramId++}`;   params.push(status); }
        if (priority) { where += ` AND t.priority = $${paramId++}`; params.push(priority); }
 ...
 */
// POST /api/tasks
router.post('/', taskCreateRules, handleValidation, async (req: any, res: Response) => {
    try {
...
router.put('/:id', taskUpdateRules, handleValidation, async (req: any, res: Response) => {
    try {
        ...
        if (assigned_to !== undefined) { updateQuery += `, assigned_to=$${paramId++}`; params.push(assigned_to); }
        if (priority    !== undefined) { updateQuery += `, priority=$${paramId++}`;    params.push(priority); }
        if (description !== undefined) { updateQuery += `, description=$${paramId++}`; params.push(description); }
        if (due_date    !== undefined) { updateQuery += `, due_date=$${paramId++}`;    params.push(due_date); }
